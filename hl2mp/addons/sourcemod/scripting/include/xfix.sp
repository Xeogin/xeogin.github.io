...
void RoundRestartCall(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_bReload = true;
...
public void OnMapStart()
{
	...
	CreateTimer(1.0, t_enablebackpackreload, _, TIMER_FLAG_NO_MAPCHANGE);
...
Action t_enablebackpackreload(Handle timer)
{
	g_bMapStart = false;
	...
	if (activator > 0 && activator <= MaxClients && IsClientInGame(activator) && IsPlayerAlive(activator))
	{
		...
		if (hClientWeapon != -1)
		{
			...
			if (g_bHasAr2[activator])
			{
				...
				// Can't use IsValidEntity for some baffling reason with the AR2 as it sees iAmmoClipOneAR2 as invalid, even if it isn't
				// (it properly prints the magazine size, even if it declares the entity as invalid).
				// So we're going to work around it by checking if the client has the weapon.
...
Action EntityOutput_SMG(const char[] output, int caller, int activator, float delay)
{
	...
	if (activator > 0 && activator <= MaxClients && IsClientInGame(activator) && IsPlayerAlive(activator))
	{
		...
		if (hClientWeapon != -1)
		{
			...
			if (IsValidEntity(iAmmoClipOneSMG))
			{
				if (hClientWeaponSMG[activator] == 0)
					return Plugin_Continue;
